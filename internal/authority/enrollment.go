...
func PrepareEnrollmentWithoutRecovery(operatorID, storeIdentity, passphrase string, now time.Time, random io.Reader) (EnrollmentPlan, error) {
	if !strings.HasPrefix(operatorID, "urn:imprint:operator:") {
		return EnrollmentPlan{}, errors.New("configured operator identity is invalid")
	}
	if !strings.HasPrefix(storeIdentity, "urn:imprint:store:") {
		return EnrollmentPlan{}, errors.New("store identity is corrupt")
	}
	if random == nil {
		random = rand.Reader
	}
	...
	installationID := "urn:imprint:installation:" + hex.EncodeToString(installationRandom)
	...
	event := GenesisEvent{
		...
		EventType: "enrollment", OperatorID: operatorID, InstallID: installationID,
		StoreIdentity: storeIdentity, KeyID: key.KeyID, PublicKeyB64: publicKeyB64,
	...
	blob, err := EncryptPrivateKey(key.PrivateKey, passphrase, KeyAAD{
		OperatorID: operatorID, InstallID: installationID, StoreIdentity: storeIdentity,
